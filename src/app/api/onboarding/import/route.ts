...
export async function POST(req: Request) {
  ...
  try {
    ...
    return NextResponse.json({ job_id: job.id, proposed_mapping: stageA.proposedMapping })
  } catch (err) {
...
export async function PATCH(req: Request) {
  ...
  await supabase.from('import_jobs')
    .update({ status: 'extracting', confirmed_mapping: body.confirmed_mapping })
    .eq('id', job.id)
  ...
  try {
    ...
    const result = await runStageB({
      ...
      churchId:         ctx.churchId,
      sources:          rawSources,
      confirmedMapping: body.confirmed_mapping,
    })

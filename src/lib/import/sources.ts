...
async function fetchGoogleSheetCsv(url: string): Promise<string> {
  ...
  if (!res.ok) {
    throw new Error(
      `Google Sheets fetch failed (${res.status}). The sheet must be published to web or link-sharing set to "Anyone with the link can view".`,
    )
  }
  const text = await res.text()
  if (text.includes('<HTML>') || text.includes('<html')) {
    throw new Error(
      'Google returned the login page instead of CSV. Set the sheet to "Anyone with the link can view" or publish it to the web.',
    )
  }
  return text
}

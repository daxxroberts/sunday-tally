...
export const WRITER_TOOLS: Anthropic.Messages.Tool[] = [
  ...
  },
  {
    ...
    description: 'Ensure a response / stat category exists. stat_scope is "audience" (per MAIN/KIDS/YOUTH) or "service" (per occurrence).',
    input_schema: {
      type: 'object',
      properties: {
        category_code: { type: 'string' },
        category_name: { type: 'string' },
        stat_scope:    { type: 'string', enum: ['audience', 'service'] },
      },

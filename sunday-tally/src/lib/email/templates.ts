  ...
  switch (template) {
    ...
    case 'invite':
      return {
        ...
        html: shell({
          d: dd,
          preheader: `${d.inviterName ?? 'A team member'} invited you to ${church} on Sunday Tally.`,
          chip: chip("You're invited", '#EEF1FE', INDIGO_DEEP),
          headline: `Join ${church} on Sunday Tally.`,
          intro: `${d.inviterName ?? 'A team member'} invited you to join <strong style="color:${INK};">${church}</strong> as <strong style="color:${INK};">${d.role ?? 'a team member'}</strong>.`,

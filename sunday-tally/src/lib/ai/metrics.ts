...
async function attendanceByWeek(ctx: MetricContext, p: Record<string, unknown>) {
  ...
  if (tagCode) {
    ...
    if (!tag) return { rows: [], shape: 'week_start: string; total: number', hint: `No tag found with tag_code "${tagCode}". Call probe_data to find valid tag codes.` }
    ...
    if (templateFilter.length === 0) return { rows: [], shape: 'week_start: string; total: number', hint: `No service templates found for tag "${tagCode}". Call probe_data to find the valid range.` }
  }
  ...
  if (rows.length === 0) {
    return {
      ...
      hint: `No attendance entries found for ${start}–${end}${tagCode ? ` with tag ${tagCode}` : ''}. Call probe_data to find the valid range.`,
    }
  }
...
async function attendanceByTemplateMonth(ctx: MetricContext, p: Record<string, unknown>) {
  ...
  if (rows.length === 0) {
    return { rows, shape: 'month: string; template_name: string; avg: number', hint: `No attendance entries found for ${year}.` }
  }
...
async function givingBySourceMonth(ctx: MetricContext, p: Record<string, unknown>) {
  ...
  if (rows.length === 0) {
    return { rows, shape: 'month: string; source_name: string; total: number', hint: `No giving entries found for ${start}–${end}.` }
  }
...
async function ytdVsPrior(ctx: MetricContext, p: Record<string, unknown>) {
  ...
  const warning = (current.weeks === 0 || prior.weeks === 0)
    ? [
        current.weeks === 0 ? `${year} YTD has no logged attendance entries.` : '',
        prior.weeks   === 0 ? `${year - 1} YTD has no logged attendance entries.` : '',
      ].filter(Boolean).join(' ')
    : undefined
...
async function volunteerCountsMonth(ctx: MetricContext, p: Record<string, unknown>) {
  ...
  if (rows.length === 0) {
    return { rows, shape: 'month: string; category_name: string; total: number', hint: `No volunteer entries found for ${start}–${end}.` }
  }
...
async function responseTotalRange(ctx: MetricContext, p: Record<string, unknown>) {
  ...
  if (rows.length === 0) {
    return { rows, shape: 'category_name: string; total: number', hint: `No response entries found for ${start}–${end}.` }
  }

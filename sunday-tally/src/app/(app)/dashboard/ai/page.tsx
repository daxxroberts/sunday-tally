...
import { useDataReview } from '@/contexts/DataReviewContext'

// ─── Chat types ───────────────────────────────────────────────────────────────

interface DataReviewPayload {
  title?: string
...
function DeltaBadge({ delta }: { delta: number | null }) {
  ...
  const up = delta >= 0
  return (
    <span className={`inline-flex items-center gap-0.5 text-[10px] font-semibold tabular-nums px-1.5 py-0.5 rounded-full ${up ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-600'
      }`}>
      {up ? '▲' : '▼'}{Math.abs(delta)}%
    </span>
  )
...
export default function AnalyticsChatPage() {
  ...
  const { open: openDataReview } = useDataReview()
  // Ref so onChoice can call the latest send() without a stale closure
  ...
  const send = useCallback(async (raw: string) => {
    ...
    function handleFrame(frame: string) {
      ...
      if (event === 'text') {
        ...
        scrollToBottom()
      } else if (event === 'data_review') {
        const dr = payload as DataReviewPayload
        openDataReview({
          title:    dr.title,
          data:     dr.data,
          columns:  dr.columns,
          ...
          choices:  dr.choices,
          onChoice: (value) => {
            const choice = dr.choices.find(c => c.value === value)
            const msg = choice?.description
  ...
  }, [busy, messages, patchLast, openDataReview])

  // Keep sendRef current so data review onChoice can always call the latest send()

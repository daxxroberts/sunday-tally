...
export default function SettingsSchedulePage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      if (sv) {
        setCurrentSchedule({ day: sv.day_of_week, time: sv.start_time, since: sv.effective_start_date })
  ...
  return (
    <AppLayout role={role}>
      <div className="sticky top-0 z-10 bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3">
        <button onClick={() => router.push('/settings/services')} className="text-gray-400 hover:text-gray-700" aria-label="Back to services">
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" /></svg>
        </button>
        <div>
          <p className="font-semibold text-gray-900 text-sm">{currentSchedule ? 'Change schedule' : 'Set schedule'}</p>
          <p className="text-xs text-gray-400">{templateName}</p>
        </div>
      </div>

      <div className="px-4 py-4 space-y-6">
        {/* E6 + E7 — current schedule as a one-line note (not a gate). Saving starts a new version. */}
        {currentSchedule && (
          <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
            <p className="text-xs text-amber-800">
              Currently <span className="font-semibold">{DAYS[currentSchedule.day]}s · {currentSchedule.time}</span> (since {currentSchedule.since}). Saving starts a new schedule from the effective date below.
            </p>
          </div>
        )}

        {/* Schedule form — shown directly, pre-filled with the current values */}
        <form onSubmit={handleSave} className="space-y-5">
          ...
          <button type="button" onClick={() => router.push('/settings/services')} className="w-full text-gray-400 text-sm py-2">Cancel</button>
        </form>
      </div>
    </AppLayout>
  )

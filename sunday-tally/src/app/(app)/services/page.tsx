...
function getChecklistLabels(card: ServiceCard, church: Church) {
  ...
  if (card.type === 'existing') {
    if (church.tracks_main_attendance) mark('Main attendance', card.main_attendance_entered)
    ...
    if (church.tracks_main_attendance) missing.push('Main attendance')
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      for (const occ of typedOccurrences) {
        ...
        let main_attendance_entered = !churchData.tracks_main_attendance
        ...
        if (attRow) {
           main_attendance_entered = churchData.tracks_main_attendance ? attRow.main_attendance !== null : true
    ...
    <AppLayout role={role}>
      ...
      <div className="sticky top-0 z-10 bg-white/95 backdrop-blur-sm border-b border-gray-100 px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Link href="/services/weekly" className="flex items-center gap-1.5 text-xs font-medium text-gray-500 hover:text-gray-900 transition-colors border border-gray-200 rounded-lg px-2.5 py-1.5 hover:border-gray-400">
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            Weekly
          </Link>
          <Link href="/services/weekly-stats" className="flex items-center gap-1.5 text-xs font-medium text-gray-500 hover:text-gray-900 transition-colors border border-gray-200 rounded-lg px-2.5 py-1.5 hover:border-gray-400">
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 19V6h13M9 6L4 11l5 5" />
            </svg>
            Stats
          </Link>
          <Link href="/services/history" className="flex items-center gap-1.5 text-xs font-medium text-gray-500 hover:text-gray-900 transition-colors border border-gray-200 rounded-lg px-2.5 py-1.5 hover:border-gray-400">
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  ...
                  <div className="space-y-8">
                    {sortedDates.map(date => {
                      ...
                      const completionCounts = existingInDay.reduce((acc, card) => {
                        if (church.tracks_main_attendance) { acc.total += 1; if (card.main_attendance_entered) acc.complete += 1 }

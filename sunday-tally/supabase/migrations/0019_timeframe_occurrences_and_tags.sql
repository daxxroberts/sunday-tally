...
-- Attendance
ALTER TABLE attendance_entries DROP CONSTRAINT uq_attendance_per_occurrence;
...
ALTER TABLE attendance_entries ADD CONSTRAINT uq_attendance_per_entry UNIQUE NULLS NOT DISTINCT (service_instance_id, occurrence_id);
...
-- Volunteers
ALTER TABLE volunteer_entries DROP CONSTRAINT uq_volunteer_entry;
...
ALTER TABLE volunteer_entries ADD CONSTRAINT uq_volunteer_entry UNIQUE NULLS NOT DISTINCT (service_instance_id, occurrence_id, volunteer_category_id);
...
-- Giving
ALTER TABLE giving_entries DROP CONSTRAINT uq_giving_entry;
...
ALTER TABLE giving_entries ADD CONSTRAINT uq_giving_entry UNIQUE NULLS NOT DISTINCT (service_instance_id, occurrence_id, giving_source_id);
...
-- Responses
ALTER TABLE response_entries DROP CONSTRAINT uq_response_entry;
ALTER TABLE response_entries RENAME COLUMN service_occurrence_id TO service_instance_id;
ALTER TABLE response_entries ADD COLUMN occurrence_id UUID REFERENCES occurrences(id) ON DELETE RESTRICT;
ALTER TABLE response_entries ALTER COLUMN service_instance_id DROP NOT NULL;
ALTER TABLE response_entries ADD CONSTRAINT uq_response_entry UNIQUE NULLS NOT DISTINCT (service_instance_id, occurrence_id, response_category_id, audience_group_code);
ALTER TABLE response_entries ADD CONSTRAINT chk_response_attachment CHECK (
  (service_instance_id IS NOT NULL AND occurrence_id IS NULL) OR
  (service_instance_id IS NULL AND occurrence_id IS NOT NULL)
);
...
ALTER TABLE instance_tags DROP CONSTRAINT service_occurrence_tags_pkey;
ALTER TABLE instance_tags ADD COLUMN id UUID PRIMARY KEY DEFAULT gen_random_uuid();
...
ALTER TABLE instance_tags ADD CONSTRAINT uq_instance_tag UNIQUE NULLS NOT DISTINCT (service_instance_id, occurrence_id, service_tag_id);

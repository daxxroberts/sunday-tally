...
- **E-9 · Metric row** — name (InlineEditField) · **mode toggle Entry / Roll up sub-entries** · Remove (amber confirm). Second line:
  - (No visible ★/"primary" marker — removed 2026-06-08 as confusing/uneditable. `is_canonical` is still auto-managed in the backend for the dashboard headline + the `uq_metric_canonical` index; it's just not surfaced. Phase B should retire "primary" in favor of summing counts for a ministry's total.)
